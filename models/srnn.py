...
class SensorRNN(BaseTFModel):
    ...
    def __init__(self, sess, config, api, log_dir, forward, scope=None):
        self.vocab_size = 32
        self.p_vocab_size = config.position_len + 1
        self.sess = sess
        self.scope = scope
        ...
        with tf.name_scope("io"):
            ...
            self.position = tf.placeholder(dtype=tf.int32, shape=(None, None), name="position")
        ...
        with variable_scope.variable_scope("wordEmbedding"):
            ...
            self.position_embedding = tf.get_variable("p_embedding", [self.p_vocab_size, config.embed_size], dtype=tf.float32)
            embedding_mask = tf.constant([0 if i == 0 else 1 for i in range(self.p_vocab_size)], dtype=tf.float32,
                                         shape=[self.p_vocab_size, 1])
            p_embedding = self.position_embedding * embedding_mask

            input_embedding = embedding_ops.embedding_lookup(embedding, self.sensor)
            position_embedding = embedding_ops.embedding_lookup(p_embedding, self.position)
        ...
        self.saver = tf.train.Saver(tf.global_variables(), write_version=tf.train.SaverDef.V2, max_to_keep=None)
    ...
    def train(self, global_t, sess, train_feed, update_limit=5000):
        ...
        for data, position, label in epoch_train:
            feed_dict = { self.sensor: data, self.output: label, self.position: position}
    ...
    def test(self, sess, test_feed):
        ...
        i = 0
        for data, position, label in epoch_test:
            feed_dict = { self.sensor: data, self.output: label, self.position: position}
